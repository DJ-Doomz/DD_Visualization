...
int START_DATE = dateToDays("2016-02-18");
int END_DATE = dateToDays("2024-10-13");
int ACTUAL_LENGTH = 3161;
float TEXT_MARGIN = 8;
...
void setup(){
  ...
  randomSeed(432766);
  textFile = loadStrings("history_final.csv");
  ...
  for(int d = 0; d < DAY_LEN; d++){
    ...
    for(int p = 0; p < PEOPLE_COUNT; p++){
      ...
      if(val < 0 && val > -1.5) // if -1 then player banned
        people[p].banned = true;
      if(val < -1.5 && val > -2.5) // -2 then player unbanned
        people[p].banned = false;
      // if player is banned, ignore all future scores
      if(people[p].banned) val = -1;
...
{
  return clamp(WAIndex(mins,d,14) - 100, 0, 1150);
}
...
void drawBackground(){
  ...
  textFont(font,110);
  textAlign(RIGHT);
  text(daysToDate(clamp(currentDay, 0, ACTUAL_LENGTH),true),width-40,130);

  // version information
  String version = "v1";
  if(currentDay + START_DATE < dateToDays("2016-07-06"))
    version = "v1";
  else if(currentDay + START_DATE < dateToDays("2016-09-19"))
    version = "v2";
  else if(currentDay + START_DATE < dateToDays("2021-02-19"))
    version = "v3";
  else if(currentDay + START_DATE < dateToDays("2021-10-27"))
    version = "v3.1";
  else
    version = "v3.2";
  textFont(font, 65);
  text(version, width-40, 200);

  // header
  fill(100);
  textAlign(CENTER);
  textFont(font,62);
  text("DD Top 10",840,Y_MIN-100);
}
...
void drawTickMarksOfUnit(int u, float alpha){
  for(int v = clampToUnit((int)currentMin, u); v < currentMin + currentScale*1.4; v+=u){
    ...
    text(Integer.toString(v),x,Y_MIN-30);
  }
}
...
void drawBars(){
  ...
  for(int p = 0; p < PEOPLE_COUNT; p++){
    ...
    if(val < 0)
      text("BANNED!!",appX,y+BAR_HEIGHT-15);
    else
    ...
    // draw time spent in top10
    textFont(font,20);
    fill(200, 200, 200, 255);
    text("Top10: " + pe.time_in_top10[(int)currentDay], X_MIN+10, y+BAR_HEIGHT-35);
    text("WR:    " + pe.total_time_WR[(int)currentDay], X_MIN+10, y+BAR_HEIGHT-15);
    // draw PB amts, fading out after a few days
    float alpha = clamp(255-15*linIndex(pe.days_since_pb,currentDay), 0, 255);
...
void getRankings(){
  for(int d = 0; d < DAY_LEN; d++){
    ...
    for(int p = 0; p < PEOPLE_COUNT; p++){
      ...
      if(d > 0)
      {
        people[p].time_in_top10[d] = people[p].time_in_top10[d-1];
        people[p].total_time_WR[d] = people[p].total_time_WR[d-1];
      }
    }
      ...
      if(d > 0)
      {
        if(spot == 0)
        {
          if(wrHolder[d-1] == people[holder].name && d < ACTUAL_LENGTH) // streak continues
          {
            ...
            people[holder].total_time_WR[d] = 1 + people[holder].total_time_WR[d-1];
          }
          else  // new WR holder
          {
            wrStreak[d] = 1;
        ...
        // keep track of time spent in top 10
        if(spot <= 9)
        {
          people[holder].time_in_top10[d] = 1 + people[holder].time_in_top10[d-1];
        }
      }
      if(spot == 9)
      {
